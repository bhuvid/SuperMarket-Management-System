    ...
    <div class="rcontent">
       <?php
        if(isset($_GET['success'])){
              $result = mysqli_query($connect,"update supplier set sname='{$_POST['name']}', saddress='{$_POST['address']}', sphone='{$_POST['phone']}', semail='{$_POST['email']}' where sdealer='{$_POST['dealer']}' ");
              if(!$result)echo "Editing not successful :(".mysqli_error();
              else echo "Editing successful :) ";
          }
       else{
        if(isset($_GET['name'])&isset($_GET['id'])&isset($_GET['deal'])){
        if(!strcmp(strtolower($_GET['name']),"supplier")) {
        $plist=mysqli_query($connect,"select * from supplier where sname='{$_GET['id']}' and sdealer='{$_GET['deal']}' ");
        $plist=mysqli_fetch_array($plist);
        echo "<form method='post' action='supplieredit.php?success=1'>
          <table>
           ...
           <input name='dealer' type='hidden' value='{$plist['sdealer']}' />
          ...
          <br>
          <button class='button-65' type='submit' value='submit' />Submit</button>
          </form>";
    }
}
       }
